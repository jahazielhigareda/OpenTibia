...
namespace OpenTibia.Game.Common
{
    public class Context : IDisposable
    {
        ...
        public Context(IClientServer server, Context previousContext)
        {
            this.server = server;
        ...
        public IClientServer ClientServer => server;
        ...
        public void AddPacket(IConnection connection, IOutgoingPacket packet)
        {
            ...
            if (server is IServer fullServer)
            {
                ...
                {
                    messageCollection = fullServer.MessageCollectionFactory.Create();

                    messageCollections.Add(connection, messageCollection);
                }

                messageCollection.Add(packet, fullServer.Features);
            }
        }
        ...
        public void Flush()
        {
            ...
            if (server is IServer fullServer && events != null)
            {
                while (events.Count > 0)
                {
                    var e = events.Dequeue();

                    foreach (var eventHandler in fullServer.EventHandlers.GetEventHandlers(e.Event) )
                    {
                        eventHandler.Handle(e.Event).Catch( (ex) =>
                        {
                            ...
                            {
                                fullServer.Logger.WriteLine(ex.ToString(), LogLevel.Error);
                            }
                        } );
                    }
                    ...
                    {
                        foreach (var eventHandler in fullServer.GameObjectEventHandlers.GetEventHandlers(e.EventSource, e.Event) )
                        {
                            eventHandler.Handle(e.Event).Catch( (ex) =>
                            {
                                if (ex is PromiseCanceledException)
                    ...
                    if (e.Position != null)
                    {
                        foreach (var eventHandler in fullServer.PositionalEventHandlers.GetEventHandlers(e.Position, e.Event) )
                        {
                            eventHandler.Handle(e.Event).Catch( (ex) =>
                            {
                                if (ex is PromiseCanceledException)
